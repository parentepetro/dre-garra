...
function tile(lbl, val, foot='', color=null, cls='', fundo=null){
  // fundo: uma cor de fundo bem clarinha, para o card que precisa saltar
  const est = fundo ? ` style="background:${fundo};border-color:${fundo}"` : '';
  return `<div class="card tile"${est}>
...
async function pageDashboard(){
  ...
  /* operação: litros só valem para combustível; na conveniência o campo é quantidade de itens */
  const faCmb = fa.filter(r=>r.segmento==='combustiveis');
  const faCnv = fa.filter(r=>r.segmento==='conveniencia');
  const litros   = sum(faCmb,'volume_litros');
  const fatCmb   = sum(faCmb,'faturamento_bruto'), lbCmb = fatCmb - sum(faCmb,'custo_mercadoria');
  const fatCnv   = sum(faCnv,'faturamento_bruto'), lbCnv = fatCnv - sum(faCnv,'custo_mercadoria');
  const itensCnv = sum(faCnv,'volume_litros');
  const precoL   = litros ? fatCmb/litros : 0;
  const margemL  = litros ? lbCmb/litros  : 0;
  const nMeses   = new Set(fa.map(r=>monthKey(r.competencia))).size || 1;

  const mLit = groupSum(faCmb, r=>monthKey(r.competencia), r=>Number(r.volume_litros)||0);
  const mMrg = meses.map(k=>{
    const f = fa.filter(r=>monthKey(r.competencia)===k);
    const ft = sum(f,'faturamento_bruto');
    return ft ? 100*(ft - sum(f,'custo_mercadoria'))/ft : 0;
  });

  const LIT = n => Number(n||0).toLocaleString('pt-BR',{maximumFractionDigits:0});
  const RL  = n => 'R$ ' + Number(n||0).toLocaleString('pt-BR',{minimumFractionDigits:3,maximumFractionDigits:3});
  ...
  $('#view').innerHTML = `
    ...
    <div class="grid tiles" style="margin-bottom:16px">
      ${tile('Litros vendidos', LIT(litros) + ' L',
        `${LIT(litros/nMeses)} L por mês em média`, PAL[6])}
      ${tile('Margem por litro', RL(margemL),
        `Preço médio de venda ${RL(precoL)}`, PAL[2])}
      ${tile('Combustíveis', money(fatCmb),
        fatCmb?`Margem ${pct(lbCmb,fatCmb)} · ${pct(fatCmb,tFat)} do faturamento`:'', PAL[3])}
      ${tile('Conveniência', money(fatCnv),
        fatCnv?`Margem <b style="margin-left:3px">${pct(lbCnv,fatCnv)}</b> · ${LIT(itensCnv)} itens`:'', PAL[0])}
    </div>
    ...
    <div class="grid" style="grid-template-columns:1.55fr 1fr;margin-bottom:16px" id="row3">
      ${vizCard({
        title:'Litros vendidos e margem por mês',
        sub:'Colunas de volume, linha de margem bruta',
        canvas:'chVol',
        legend: legendHTML([{c:PAL[6],t:'Litros',v:litros},{c:PAL[2],t:'Margem bruta',v:tLB}]),
        table: miniTable(
          [{t:'Mês'},{t:'Litros',num:1},{t:'Faturamento',num:1},{t:'Margem',num:1},{t:'R$/litro',num:1}],
          meses.map((k,i)=>{
            const l = mLit.get(k)||0, f = mFat.get(k)||0, lb = mLB.get(k)||0;
            return [monthLabelLong(k), LIT(l)+' L', money(f), mMrg[i].toFixed(2)+'%',
                    l?RL(lb/l):'—'];
          }),
          ['Total', LIT(litros)+' L', money(tFat), tFat?pct(tLB,tFat):'—', RL(margemL)])
      })}
      ${vizCard({
        title:'Combustíveis × Conveniência',
        sub:'Quanto cada um vende e quanto cada um deixa',
        canvas:'chSeg',
        legend: legendHTML([{c:PAL[3],t:'Combustíveis',v:lbCmb},{c:PAL[0],t:'Conveniência',v:lbCnv}]),
        table: miniTable([{t:'Segmento'},{t:'Faturamento',num:1},{t:'Lucro bruto',num:1},{t:'Margem',num:1}],
          [['Combustíveis', money(fatCmb), money(lbCmb), fatCmb?pct(lbCmb,fatCmb):'—'],
           ['Conveniência', money(fatCnv), money(lbCnv), fatCnv?pct(lbCnv,fatCnv):'—']],
          ['Total', money(tFat), money(tLB), tFat?pct(tLB,tFat):'—'])
      })}
    </div>
  ...
  /* --- volume por mes, com a margem sobreposta --- */
  S.charts.vol = new Chart($('#chVol'), {
    type:'bar',
    data:{ labels: meses.map(monthLabel), datasets:[
      { type:'bar', label:'Litros', data: meses.map(k=>mLit.get(k)||0), backgroundColor:PAL[6],
        borderRadius:{topLeft:4,topRight:4}, borderSkipped:false, borderColor:INK.surface,
        borderWidth:{left:1,right:1}, maxBarThickness:30, yAxisID:'y' },
      { type:'line', label:'Margem', data: mMrg, borderColor:PAL[2], backgroundColor:PAL[2],
        borderWidth:2, pointRadius:3, pointHoverRadius:5, tension:.3, yAxisID:'y2' }
    ]},
    options:{ plugins:{legend:{display:false},
      tooltip:{...tooltipCfg, callbacks:{label:c=> c.datasetIndex===0
        ? ` ${LIT(c.parsed.y)} litros` : ` margem ${c.parsed.y.toFixed(2)}%`}}},
      interaction:{mode:'index',intersect:false},
      scales:{
        x: axisCat,
        y: {...axisCat, beginAtZero:true, position:'left',
            ticks:{...(axisCat.ticks||{}), callback:v=>(v/1000).toLocaleString('pt-BR')+' mil'}},
        y2:{ position:'right', beginAtZero:true, grid:{drawOnChartArea:false},
             ticks:{color:INK.muted, font:{size:11}, callback:v=>v.toFixed(0)+'%'} }
      } }
  });

  /* --- combustivel x conveniencia --- */
  S.charts.seg = new Chart($('#chSeg'), {
    type:'bar',
    data:{ labels:['Combustíveis','Conveniência'], datasets:[
      { label:'Lucro bruto', data:[lbCmb, lbCnv], backgroundColor:[PAL[3],PAL[0]],
        borderRadius:{topLeft:4,topRight:4}, borderSkipped:false, borderColor:INK.surface,
        borderWidth:{left:1,right:1}, maxBarThickness:64 }
    ]},
    options:{ plugins:{legend:{display:false},
      tooltip:{...tooltipCfg, callbacks:{label:c=>` ${money(c.parsed.y)} de lucro bruto`}}},
...
async function pageAcerto(){
  const A = S.sub.acerto = S.sub.acerto || { vis:'posto', incluirQuitados:false, tipo:'todos' };
  ...
  const todos = data || [];
  const movBase = A.incluirQuitados ? todos : todos.filter(m=>m.status!=='quitado');

  const abertos = todos.filter(m=>m.status!=='quitado');
  const sinal = m => m.direcao==='entrou' ? 1 : -1;   // + = entrou no Garra, como no extrato
  ...
  const dinSaiu   = sum(abertos.filter(m=>m.tipo==='dinheiro' && m.direcao==='saiu'));
  const dinEnt    = sum(abertos.filter(m=>m.tipo==='dinheiro' && m.direcao==='entrou'));
  const saldoDin  = dinEnt - dinSaiu;                       // + = entrou no Garra
  const saldoComb = saldoDe(abertos.filter(m=>m.tipo==='combustivel'));
  ...
  // filtro de tipo: vale para as tabelas e os graficos, nao para os cards
  const doTipo = arr => A.tipo==='todos' ? arr : arr.filter(m=>m.tipo===A.tipo);
  const abertosF = doTipo(abertos);
  const mov = doTipo(movBase);

  const ids = [...new Set(abertosF.map(m=>m.posto_id))];
  const porPosto = ids.map(id => {
    const meus = abertosF.filter(m=>m.posto_id===id);
  ...
  $('#view').innerHTML = `
    ${roNote()}
    <div class="grid tiles" style="margin-bottom:16px">
      ${tile('Dinheiro', money(Math.abs(saldoDin)),
        `Saiu ${money(dinSaiu)} · voltou ${money(dinEnt)}<br>
         <b>${saldoDin >= 0 ? 'o Garra deve' : 'a receber dos postos'}</b>`,
        saldoDin>=0?PAL[2]:PAL[1], saldoDin>=0?'pos':'neg')}

      ${tile('Combustível', money(Math.abs(saldoComb)),
        `${litrosFmt(litrosAberto)} em aberto${litrosSemPreco>0?` · ${litrosFmt(litrosSemPreco)} sem preço`:''}<br>
         <b>${saldoComb >= 0 ? 'o Garra deve' : 'a receber dos postos'}</b>`,
        saldoComb>=0?PAL[2]:PAL[3], saldoComb>=0?'pos':'neg')}

      ${tile(saldo >= 0 ? 'O Garra deve aos postos' : 'Os postos devem ao Garra',
        money(Math.abs(saldo)), 'Entrou menos saiu, dinheiro e combustível juntos',
        saldo>=0?PAL[2]:PAL[1], saldo>=0?'pos':'neg',
        saldo>=0 ? 'color-mix(in srgb, var(--good) 9%, var(--surface))'
                 : 'color-mix(in srgb, var(--critical) 9%, var(--surface))')}
    </div>

    <div class="msg info" style="margin-bottom:16px">
      <b>O acerto é em reais.</b> Cada empréstimo de combustível entra pelo valor da nota do dia em que
      foi feito, e esse valor fica congelado — o que o preço fizer depois não muda mais a dívida.
      Por isso dinheiro e combustível somam na mesma conta, e o card da diferença é o número que vale.
    </div>

    ${litrosSemPreco > 0 ? `<div class="msg" style="margin-bottom:16px;border-left:3px solid var(--warning)">
      <b>${litrosFmt(litrosSemPreco)}</b> de combustível ainda estão <b>sem valor por litro</b>, então entram
      no saldo como zero. Como o acerto é em reais, esses litros não estão sendo cobrados —
      abra cada um e preencha a nota.
    </div>`:''}

    ${porPosto.length ? `<div class="grid" style="margin-bottom:16px">
      ${vizCard({ title:'Saldo por posto', sub:'Positivo: entrou no Garra · negativo: saiu', canvas:'chAcSaldo',
    ...
    ${toolbar(`<div class="sub-tabs" style="margin:0">
        ${[['todos','Todos'],['combustivel','Combustível'],['dinheiro','Dinheiro']].map(([v,t])=>
          `<button data-tp="${v}" class="${A.tipo===v?'on':''}">${t}</button>`).join('')}
      </div>
      <label style="display:flex;gap:7px;align-items:center;font-size:12.5px;color:var(--text-secondary);cursor:pointer">
    ...
    ${A.vis==='posto' ? `
      ...
      <tbody id="tbody">${porPosto.length ? porPosto.map(p=>`
        <tr data-s="${esc(p.nome.toLowerCase())}">
          ...
          <td><span class="pill"><i style="background:${p.saldo>=0?'var(--good)':'var(--critical)'}"></i>${p.saldo>=0?'o Garra deve':'deve ao Garra'}</span></td>
        </tr>`).join('') : emptyRow(6,'Nenhuma movimentação em aberto.')}</tbody>
      ${porPosto.length?`<tfoot><tr><td>Total</td>
        <td class="num">${money(sum(abertosF.filter(m=>m.tipo==='dinheiro'&&m.direcao==='saiu')))}</td>
        <td class="num">${money(sum(abertosF.filter(m=>m.tipo==='dinheiro'&&m.direcao==='entrou')))}</td>
        <td class="num">${litrosFmt(sum(abertosF.filter(m=>m.tipo==='combustivel'),'litros'))}</td>
        <td class="num"><b class="${porPosto.reduce((a,p)=>a+p.saldo,0)>=0?'pos':'neg'}">${money(porPosto.reduce((a,p)=>a+p.saldo,0))}</b></td><td></td></tr></tfoot>`:''}
    ` : `
      ...
      <tbody id="tbody">${mov.length ? mov.map(m=>`
        <tr data-s="${esc((nomePosto(m.posto_id)+' '+(m.descricao||'')+' '+(PRODUTO[m.produto]||'')+' '+brDate(m.data)).toLowerCase())}"
          ...
          <td class="num"><b class="${m.direcao==='entrou'?'pos':'neg'}">${m.direcao==='entrou'?'+':'−'} ${money(m.valor)}</b></td>
      ...
      ${mov.length?`<tfoot><tr><td colspan="6">Entrou menos saiu no período listado</td>
        <td class="num"><b class="${(sum(mov.filter(m=>m.direcao==='entrou'))-sum(mov.filter(m=>m.direcao==='saiu')))>=0?'pos':'neg'}">${money(sum(mov.filter(m=>m.direcao==='entrou'))-sum(mov.filter(m=>m.direcao==='saiu')))}</b></td>
        <td colspan="2"></td></tr></tfoot>`:''}
    `}
    </table></div></div>`;

  bindVizToggles(); wireSearch();
  $$('.sub-tabs button[data-v]').forEach(b=>b.addEventListener('click',()=>{ A.vis = b.dataset.v; render(); }));
  $('#cbQuit')?.addEventListener('change', e=>{ A.incluirQuitados = e.target.checked; render(); });
  $$('[data-tp]').forEach(b=>b.addEventListener('click', ()=>{ A.tipo = b.dataset.tp; render(); }));
